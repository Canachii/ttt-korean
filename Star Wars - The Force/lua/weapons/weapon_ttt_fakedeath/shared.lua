...
function SWEP:Initialize()
	...
	if CLIENT then
		if TTT2 then
			self:AddTTT2HUDHelp("시체 스폰", "빠르게 시체의 역할 변경")
			self:AddHUDHelpLine("시체 설정 (이름, 역할, 사인)", Key("+reload", "R"))
		else
			self:AddHUDHelp("우클릭으로 빠르게 시체의 역할 변경", "[R]로 시체 설정 (이름, 역할, 사인)", false)
		end
	end
end
